	...
	describe('Functions tests', () => {
		it('test onInputChange function', () => {
			...
			component
				...
				.at(0)
				.simulate('change', { target: { name: 'type', value: 1 } });
			expect(component.state('type')).toBe(1);

			component
				.find('input[name="title"]')
				.simulate('change', { target: { name: 'title', value: 'CityName' } });
			expect(component.state('title')).toBe('CityName');

			component
				.find('input[name="address"]')
				.simulate('change', { target: { name: 'address', value: 'Address' } });
			expect(component.state('address')).toBe('Address');

			component
				.find('input[name="price"]')
				.simulate('change', { target: { name: 'price', value: 25 } });
		...
		it('test onSubmit function', async () => {
			...
			await waitFor(() =>
				expect(historyMock.replace.mock.calls[0]).toEqual(['/posts/1'])
			);
